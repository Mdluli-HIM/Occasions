...
type DashboardStats = {
  ...
  profileTasks: { label: string; completed: boolean }[];
  leadSummary: {
    new: number;
    viewed: number;
    contacted: number;
...
export default async function ProviderDashboardPage() {
  const [stats, leads] = await Promise.all([
    apiServer<DashboardStats>("/api/providers/me/stats"),
  ...
  return (
    ...
    >
      ...
      <section className="mt-6 rounded-[26px] border border-[#deded9] bg-white p-6 shadow-sm">
        <h2 className="text-lg font-black">Leads by status</h2>
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-7">
          {(
            [
              ["New", leadSummary.new],
              ["Viewed", leadSummary.viewed],
              ["Contacted", leadSummary.contacted],
              ["Quoted", leadSummary.quoted],
              ["Accepted", leadSummary.accepted],
              ["Completed", leadSummary.completed],
              ["Closed", leadSummary.closed],
            ] as [DashboardLead["status"], number][]
          ).map(([status, count]) => (
            <div
              key={status}
              className="rounded-[16px] border border-[#eee8e3] bg-[#fbfbfa] p-3 text-center"
            >
              <p className={`inline-block rounded-full px-2 py-0.5 text-[11px] font-black ${LEAD_STATUS_STYLES[status]}`}>
                {status}
              </p>
              <p className="mt-2 text-2xl font-black text-[#111111]">{count}</p>
            </div>
          ))}
        </div>
      </section>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`rounded-full px-3 py-1 text-xs font-black ${LEAD_STATUS_STYLES[lead.status]}`}>
                        {lead.status}
                      </span>
        ...
        <aside className="space-y-6">
          <section className="rounded-[26px] border border-[#deded9] bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 h-3 overflow-hidden rounded-full bg-[#f1ece8]">
              <div
                className="h-full rounded-full bg-[#ff5a40]"
