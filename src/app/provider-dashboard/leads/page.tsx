...
export default async function ProviderLeadsPage() {
  const leads = await apiServer<DashboardLead[]>("/api/providers/me/leads");
    ...
    >
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <LeadMetric
          label="Quoted"
          value={String(
            leads.filter((lead) => lead.status === "Quoted").length,
          )}
        />
      </div>
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <Link
                  ...
                  className="mt-5 inline-block text-2xl font-black transition hover:text-[#ff5a40]"
                >
                  {lead.name}
                </Link>
              ...
              <div className="grid min-w-[240px] gap-3">
                ...
                <form action={markContacted}>
                  ...
                  <button
                    type="submit"
                    className="inline-flex min-h-[48px] w-full items-center justify-center rounded-[14px] border border-[#deded9] bg-[#f6f6f4] px-5 text-sm font-black text-[#596273] transition hover:border-[#ff5a40] hover:text-[#ff5a40]"
                  >
                    Mark as followed up
                  </button>
                </form>
