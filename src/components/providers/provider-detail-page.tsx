...
export function ProviderDetailPage({ provider }: { provider: ProviderDetail }) {
  ...
  useEffect(() => {
    ...
    apiClient<{ id: string }[]>("/api/me/saved-providers")
      .then((items) => setSaved(items.some((item) => item.id === provider.id)))
      .catch(() => {});
  }, [session?.user, provider.id]);

  function toggleSaved() {
    const next = !saved;
    setSaved(next);

    if (!session?.user) return; // guests: local-only heart

    const request = next
      ? apiClient(`/api/me/saved-providers/${provider.id}`, { method: "POST" })
      : apiClient(`/api/me/saved-providers/${provider.id}`, { method: "DELETE" });

    request.catch((err) => console.error("Failed to update saved provider:", err));
  }
    ...
    <section className="overflow-hidden rounded-[28px] bg-white shadow-[0_18px_50px_rgba(17,17,17,0.12)]">
      <div className="grid gap-1 bg-[#111111] md:grid-cols-[2fr_0.9fr]">
        ...
        >
          ...
          <div className="absolute bottom-6 left-6 rounded-full bg-white/95 px-4 py-2 text-sm font-black text-[#111111] shadow-lg transition group-hover:bg-[#ff5a40] group-hover:text-white">
          ...
          {sideImages.map((image, index) => {
            ...
            return (
              ...
              >
                ...
                {index === 1 ? (
                  <div className="absolute bottom-5 right-5 rounded-[16px] bg-white px-5 py-3 text-sm font-black text-[#111111] shadow-lg transition group-hover:bg-[#ff5a40] group-hover:text-white">
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        {questions.map((question) => (
          <button
            key={question}
            type="button"
            className="rounded-full border border-[#ff5a40] bg-white px-4 py-2 text-sm font-bold text-[#111111] transition hover:bg-[#ff5a40] hover:text-white"
          >
            {question}
          </button>
        ))}
...
function ContactCard({ provider }: { provider: ProviderDetail }) {
  ...
  return (
    <>
      <section id="contact" className="rounded-[24px] border border-[#deded9] bg-white p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="block">
              ...
              >
                ...
                <option value="R30 000 - R50 000">R30 000 - R50 000</option>
          ...
          {error ? (
            <p className="text-sm font-bold text-[#ff5a40]">{error}</p>
          ) : null}
      ...
      <div className="w-full max-w-xl overflow-hidden rounded-[28px] bg-white shadow-[0_30px_90px_rgba(17,17,17,0.35)]">
        ...
        <div className="space-y-4 p-7">
          ...
          <p className="text-sm font-bold leading-6 text-[#6b7280]">
            This request is now in the provider's dashboard. Most providers respond within a day.
          </p>
