import { Bell, ChevronRight, Menu, UserRound } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#ece7e2] bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/90">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 md:px-8">
        <button
          className="inline-flex h-11 w-11 items-center justify-center rounded-xl text-[#111111] md:hidden"
        ...
        <div className="hidden items-center gap-4 md:flex">
          <Button variant="outline" className="min-h-11 rounded-2xl border-[#ece7e2] px-6">
            List Your Business
          </Button>
      ...
      <div className="pointer-events-none invisible absolute left-1/2 top-full z-[70] w-[920px] -translate-x-1/2 pt-4 opacity-0 transition duration-200 group-hover:pointer-events-auto group-hover:visible group-hover:opacity-100">
        <div className="overflow-hidden rounded-[28px] border border-[#ece7e2] bg-white shadow-[0_28px_90px_rgba(17,17,17,0.12)]">
          <div className="grid grid-cols-[280px_1fr]">
            ...
            <div className="grid gap-8 p-7" style={{ gridTemplateColumns: `repeat(${Math.min(item.groups.length, 3)}, minmax(0, 1fr))` }}>
                    ...
                    {group.links.map((link) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <ChevronRight
                            size={15}
                            className="mt-0.5 shrink-0 text-[#c7b9ae] transition group-hover:text-[#ff5a40]"
                          />
                        </div>
                      </Link>
                    ))}
